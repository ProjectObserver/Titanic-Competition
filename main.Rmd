...
#### function to add information by name into data set
...
lm_Age <- function(ds) {
  ds_m <- ds[!is.na(ds$Age),]
  lr_Age<- lm(I(log(Age+0.0001))~isSingle*title + isSingle*SibSp + Parch, data=ds_m)
...
trf = 
  train(
    ...
    data=dtr, method="rf", metric="Kappa", trControl=ctrl, verbose=FALSE, ntree=400
  )
...
tsvm = 
  train(
    Survived ~  Sex + p_Survived + cnt_ticket + title + Pclass + CabinCnt + CabinLet + I(CabinLet=="") + AgeDecile + SibSp + Parch + Embarked + 
              familySz + nFare +isSinglenoSibSp:r_SibSp_Parch, 
    data=dtr, method="svmPoly",tuneLength=5, metric="Kappa", trControl=ctrl, verbose=FALSE, preProc = c("center", "scale")
  )
