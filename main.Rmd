...
ggplot(ds_m[ds_m$Pclass==3,], aes(x=(nFare), fill=AgeDecile, colour=AgeDecile)) + geom_density(adjust=1, alpha=0.1)
...
md_Age <- function(ds) {
  ...
  #ds_m <- ds[!is.na(ds$Age),]
  trf_Age<- randomForest(I(log(Age+0.0001)) ~ title + familySz + SibSp + Parch + Pclass + nFare  + CabinLet + isSinglenoSibSp + 
                        I(SibSp==1 & Parch==0) +  
                        I((SibSp+0.01)/(SibSp+Parch+0.01))  , data=ds_m, importance=TRUE,proximity=TRUE,ntree=500)
  print(varImp(trf_Age))

#  trf_Age<- lm(Age~isSingle*title + isSingle*SibSp + familySz + Pclass + nFare + I((SibSp+0.01)/(SibSp+Parch+0.01)) , data=ds_m)
#  print(summary(trf_Age))                      
  # check fit
  return(trf_Age)
}

#### function to do sampling by Age
##### Want to emphasis the very young (<11) who has high survival rate and old (>58) has low survival rate.
##### maintain the 17<x<30 which most of them died
##### Therefore, oversampling those interested grps. 
##### Return data set with equal number of samples from each group.
reSamplebyAge <- function(ds, gpcut){
  ds$AgeGp <- cut(ds$Age, gpcut)
  ds$isAge1<-as.factor(ds$AgeGp == names(table(ds$AgeGp))[1])
  ds$isAge3<-as.factor(ds$AgeGp == names(table(ds$AgeGp))[3])
  ds$isAgelt<-as.factor (ds$AgeGp == names(table(ds$AgeGp))[length(table(ds$AgeGp))])
  ds1 <-SMOTE(isAge1 ~ Sex + Pclass, ds, perc.over = 800, k =5, perc.under=160)
  ds3 <-SMOTE(isAge3 ~ Sex + Pclass, ds, perc.over = 4, k =5, perc.under=0)
  dslt <-SMOTE(isAgelt ~ Sex + Pclass, ds, perc.over = 2000, k =5, perc.under=0)
  dsampled <- rbind (ds3, ds1,dslt)
  print(table(dsampled$AgeGp))
  return(dsampled)
}

#### function to create variables after the Age prediction
psttreatment <- function(trf_Age, ds) {
  ds$AgePred <- exp(predict(trf_Age, ds))
  ds$AgeDecile = cut(ds$Age, c(-2000,11,17,30,60,200))
  ds$AgePredDecile = cut(ds$AgePred, c(-2000,11,17,30,60,200))
  print(table(ds$AgeDecile[!is.na(ds$Age)],ds$AgePredDecile[!is.na(ds$Age)]))
  # replace NA
...
trf_Age <- md_Age(dall)
ggplot(dall, aes(title,Age)) + geom_jitter() + geom_point()
ggplot(dall, aes(Survived,title)) + geom_jitter() + geom_point()
...
#method="svmLinear",
tsvm = train(Survived ~  p_Survived + title + Pclass + CabinLet + AgeDecile + familySz + nFare, data=dtr, method="svmLinear",tuneLength=5, metric="Kappa", trControl=ctrl, verbose=FALSE)
