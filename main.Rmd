...
#library(ggplot2)
#ggplot(ds, aes(title,Age)) + stat_smooth() + geom_point()
...
#ggplot(ds, aes(Survived,Age)) + stat_smooth() + geom_point()
#ggplot(ds, aes(Fare,Age)) + stat_smooth() + geom_point()
#ggplot(ds, aes(x=SibSp, colour=Survived)) + geom_density()
##ggplot(ds, aes(Survived,Embarked)) +geom_jitter()+ stat_smooth() + geom_point()


ds$noSibSp = (ds$SibSp == 0)
...
ds$AgePredDecile = cut(ds$AgePred, c(-2000,14,19,22,25,28,32,36,41,50,200))
...
trf = train(Survived ~ Sex + CabinLet:Pclass  + AgeDecile:SibSp + title:isSingle, data=ds, method="rf", metric="Kappa", trControl=ctrl, subset = in_train, verbose=FALSE, ntree=500)
