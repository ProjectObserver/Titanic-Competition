...
pretreatment <- function(ds){
  ...
  # Group all people in the same Cabin 
  df<-table(ds$Cabin)
...
md_Fare <- function(de) {
#  trf_Fare<- randomForest(I(log(Fare+0.001))~ title + Pclass + CabinLet + CabinNum + Embarked + Age, data=de, importance=TRUE,proximity=TRUE,ntree=700) CabinLet + CabinNum + 
...
dtr$nsample[dtr$GrpTicket == "XXXXXXX"] = dtr$nsample[dtr$GrpTicket == "XXXXXXX"] / 500
dtr$SurvivedTotal[dtr$GrpTicket == "XXXXXXX"] = dtr$SurvivedTotal[dtr$GrpTicket == "XXXXXXX"] / 500
...
tsvm = 
  train(
    ...
    data=dtr, method="svmLinear",tuneLength=10, metric="Kappa", trControl=ctrl, verbose=FALSE, preProc = c("center", "scale")
  )
...
dtest$nsample[dtest$GrpTicket == "XXXXXXX"] = dtest$nsample[dtest$GrpTicket == "XXXXXXX"] / 500
dtest$SurvivedTotal[dtest$GrpTicket == "XXXXXXX"] = dtest$SurvivedTotal[dtest$GrpTicket == "XXXXXXX"] / 500
