...
generate_BAY_p_ds<-function(i, dtest){
  p <- rbeta(1, dtest$SurvivedTotal[i] + 1, dtest$nsample[i] - dtest$SurvivedTotal[i] + 3 )
  return (p)
}
...
add_info_by_name <- function(dtr, name_, lookup){
  #add_info_by_name(dtr,c('GrpTicket','isMr'), lookup)
  #add_info_by_name(test,c('GrpTicket','isMr'), lookup)
...
md_Age <- function(ds) {
  ds_m <- ds[!is.na(ds$Age),]
  ds_extra <- ds_m[ds_m$Age>60,]
  ds_m <- rbind(ds_m, ds_extra, ds_extra)
  trf_Age<- randomForest(I(log(Age+0.0001))~ title + isSingle*SibSp + I(Parch<=2):Parch:I(SibSp>1) + Parch:familySz + SibSp + Pclass + r_SibSp_Parch, data=ds_m, importance=TRUE,proximity=TRUE,ntree=500)
...
ggplot(dall, aes(AgePred, Age)) + geom_point(alpha=0.1) + geom_smooth(method='lm',formula=y~x)
...
in_train = createDataPartition(ds$Survived, p=.99, list=FALSE)
...
dtr$p_S_cut = cut(dtr$p_Survived, seq(0,1,0.2))
...
tgbm2 = 
  train(
    Survived ~  Sex + p_S_cut + cnt_ticket + title + Pclass + CabinLet + AgeDecile + SibSp + Parch + Embarked + 
              familySz + Pclass:nFare +isSinglenoSibSp:r_SibSp_Parch, data=dtr, method="gbm", 
...
trf = 
  train(
    Survived ~  Sex + p_S_cut + cnt_ticket + title + Pclass + CabinLet + AgeDecile + SibSp + Parch + Embarked + 
              familySz + Pclass:nFare +isSinglenoSibSp:r_SibSp_Parch, 
    data=dtr, method="rf", metric="Kappa", trControl=ctrl, verbose=FALSE, ntree=500
  )
...
test<-add_info_by_name(test,c('GrpTicket','isMr'), lookup)
test$SurvivedTotal[is.na(test$nsample)]=0
test$nsample[is.na(test$nsample)]=0

#backgroud survived rate = Beta(1,1)

pred.tgbm2 <- rep(0,nrow(test))
pred.rf <- rep(0,nrow(test))
pred.svm <- rep(0,nrow(test))
pred.vote <- rep(0,nrow(test))

for (i in 1:99){
  test$p_Survived<-sapply(1 : nrow(test), generate_BAY_p_ds, dtest =test)
  test$p_S_cut = cut(test$p_Survived, seq(0,1,0.2))
  test<-run_prediction(tgbm2, trf, tsvm, test)
  pred.tgbm2 <- pred.tgbm2 + as.integer(test$pred.tgbm2) - 1
  pred.rf <- pred.rf + as.integer(test$pred.rf) - 1
  pred.svm <- pred.svm + as.integer(test$pred.svm) - 1
  pred.vote <- pred.vote + as.integer(test$pred.vote) - 1
}
test$pred.tgbm2 = as.integer(pred.tgbm2>=50)
test$pred.rf = as.integer(pred.rf>=50)
test$pred.svm = as.integer(pred.svm>=50)
test$pred.vote = as.integer(pred.vote>=50)
...
#backgroud survived rate = Beta(1,1)
## Run the data through the model
pred.tgbm2 <- rep(0,nrow(dtest))
pred.rf <- rep(0,nrow(dtest))
pred.svm <- rep(0,nrow(dtest))
pred.vote <- rep(0,nrow(dtest))

for (i in 1:99){
  dtest$p_Survived<-sapply(1 : length(dtest$nsample), generate_BAY_p_ds, dtest =dtest)
  dtest$p_S_cut = cut(dtest$p_Survived, seq(0,1,0.2))
  dtest<-run_prediction(tgbm2, trf, tsvm, dtest)
  pred.tgbm2 <- pred.tgbm2 + as.integer(dtest$pred.tgbm2) - 1
  pred.rf <- pred.rf + as.integer(dtest$pred.rf) - 1
  pred.svm <- pred.svm + as.integer(dtest$pred.svm) - 1
  pred.vote <- pred.vote + as.integer(dtest$pred.vote) - 1
}
dtest$pred.tgbm2 = as.integer(pred.tgbm2>=50)
dtest$pred.rf = as.integer(pred.rf>=50)
dtest$pred.svm = as.integer(pred.svm>=50)
dtest$pred.vote = as.integer(pred.vote>=50)
